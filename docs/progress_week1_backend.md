...
- Thiết lập dự án **Spring Boot 3.2.5** viết hoàn toàn bằng **Kotlin** với Gradle Kotlin DSL.
...
Các bảng dữ liệu được tạo tự động thông qua Flyway migration file `V1__init_auth_schema.sql`:
...
### Bước 3: Kiểm thử trên Swagger UI
...
- **Thử nghiệm bảo mật:** Copy chuỗi `accessToken`, click vào nút **Authorize** ở góc phải trên Swagger UI, paste token vào dưới dạng `Bearer <token>` ➔ Gọi thử API `/api/v1/users/me` để kiểm tra khả năng phân quyền và giải mã dữ liệu của Backend.
